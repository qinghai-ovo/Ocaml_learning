...
module Evaluator = struct
  ...
  (*Q3:make Ast to Prolog string*)
  let rec ast_to_prolog_string ast = match ast with
    | App (s, args) -> 
        let args_str = String.concat ", " (List.map ast_to_prolog_string args) in
        Printf.sprintf "%s(%s)" s args_str
    | Atom s -> s
    | Var s -> s

  let print_prolog_code str = Printf.printf "%s\n" str

  (*Q4: extract vars from Ast*)
  let rec extract_vars ast =
    let rec aux ast acc = match ast with
      | Var v -> if List.mem v acc then acc else v :: acc
      | App (_, args) -> List.fold_left (fun acc arg -> aux arg acc) acc args
      | Atom _ -> acc
    in aux ast []
  ...
  (*Q3: print as prolog code*)
  let succeed query ext_vars= 
    let str = ast_to_prolog_string query in
    print_prolog_code str;
    List.iter (fun var ->
      match var with
      | Var(name) -> 
        let value = ast_to_prolog_string (sub name (Var name) result) in
        Printf.printf "%s = %s\n" name value
      | _ -> ()) ext_vars;
    true

  let rec solve (program, question, result, depth, ext_vars) = match question with
      | [] -> succeed result ext_vars
      | goal::goals ->
                  ...
                  let (unifiable, unifier) = mgu(head, goal) in
                    if unifiable then
                      solve (program, List.map unifier (conds@goals),
                              unifier result, depth+1, ext_vars)
                    else true
            in List.fold_left onestep true program

  let eval (program, question) = 
    let ext_vars = extract_vars question in 
    solve(program, [question], question, 1, ext_vars)
