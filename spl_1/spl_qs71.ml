...
module Evaluator = struct
 ...
 (*Q4 Print Var = value yes*)
  ...
  (*let rec succeed_command var = 
    L._ISTREAM := stdin;reset_tok(); advance (); match !tok with
  ...
  (*let eval (program, question) = 
    release_var();
    solve(program, [question], (let _ = extract_Var question in !ext_var) ,1)*)
...
module Parser = struct
  ...
  and term() = match !tok with
    ...
    | L.VID s -> (eat(L.VID s); eat(L.IS); E.App("is", E.Var(s)::[arithmexp()]))
    | _ -> predicate()
  ...
  and arithmexp() = 
    let term1 = arithmterm() in
      match !tok with
      | L.ONE '+' -> (eat(L.ONE '+'); let term2 = arithmexp() in E.App("+", [term1; term2]))
      | L.ONE '-' -> (eat(L.ONE '-'); let term2 = arithmexp() in E.App("-", [term1; term2]))
      | _ -> term1

  and arithmterm() = 
    let factor1 = arithmfactor() in
      match !tok with
      | L.ONE '*' -> (eat(L.ONE '*'); let factor2 = arithmterm() in E.App("*", [factor1; factor2]))
      | L.ONE '/' -> (eat(L.ONE '/'); let factor2 = arithmterm() in E.App("/", [factor1; factor2]))
      | _ -> factor1

  and arithmfactor() = match !tok with
    | L.ONE '(' -> (eat(L.ONE '('); let exp = arithmexp() in eat(L.ONE ')'); exp)
    | L.ONE '-' -> (eat(L.ONE '-'); let exp = arithmexp() in E.App("-", [exp]))
    | L.CID s -> (eat(L.CID ""); let tail = tail_opt() in match tail with
                  | None -> E.Atom s
                  | Some a -> E.App(s, a))
